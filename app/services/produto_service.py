...
def _registrar_preco(db: Session, produto_id: int, preco) -> None:
    """Grava um snapshot do preco no historico. Chamado no cadastro e sempre
    que o preco muda numa atualizacao — nunca direto pelo usuario."""
    db.add(HistoricoPreco(produto_id=produto_id, preco=preco))
...
def cadastrar(db: Session, req: CadastroProdutoRequest, afiliado_id: int) -> ProdutoResponse:
    ...
    db.add(produto)
    db.flush()  # garante produto.id antes de gravar o historico
    _registrar_preco(db, produto.id, produto.preco)
    db.commit()
    db.refresh(produto)
